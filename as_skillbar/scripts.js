$(document).ready(function(){
    ...
    document.onkeydown = function (data) {
        ...
        if (data.which == 69) {
          closeMain()
          $.post('http://as_skillbar/taskEnd', JSON.stringify({taskResult: percent}));
        }
    }
    ...
    window.addEventListener('message', function(event){
      ...
      if(item.runUpdate === true) {
        ...
        if (item.Length < (item.chance + item.skillGap) && item.Length > (item.chance)) {
          $('.skillprogress').css("background-color","rgba(255,255,255,1)");
  
        } else {
          $('.skillprogress').css("background-color","rgba(0,0,0,1)");
        }
      ...
      if(item.closeFail === true) {
        closeMain()
        $.post('http://as_skillbar/taskCancel', JSON.stringify({tasknum: curTask}));
      }
